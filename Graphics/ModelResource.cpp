	...
	UINT dummy_color)
{
	...
	// テクスチャ読み込み
	HRESULT hr = Texture::LoadTexture(device, filepath, srv, nullptr);
...
void ModelResource::BuildModel(ID3D11Device* device, const char* dirname)
{
	for (Material& material : materials)
	{
		...
		// メタルネススムーズネステクスチャ読み込み
		LoadTexture(device, filename, "_MS", true, material.metalness_smoothness_map.GetAddressOf(), 0x00FFFF00);

		// 遮蔽テクスチャ読み込み
		LoadTexture(device, filename, "_O", true, material.occlusion_map.GetAddressOf(), 0xFFFFFFFF);
	}
