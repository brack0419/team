...
DepthOfFieldShader::DepthOfFieldShader(ID3D11Device* device)
{
	// 頂点シェーダー
	{
		// ファイルを開く
		FILE* fp = nullptr;
		fopen_s(&fp, "Shader\\DepthOfFieldVS.cso", "rb");
	...
	// ピクセルシェーダー
	{
		// ファイルを開く
		FILE* fp = nullptr;
		fopen_s(&fp, "Shader\\DepthOfFieldPS.cso", "rb");
		_ASSERT_EXPR_A(fp, "CSO File not found");
	...
	// 定数バッファ
	{
		...
		desc.MiscFlags = 0;
		desc.ByteWidth = sizeof(CBDepthOfField);
		desc.StructureByteStride = 0;

		HRESULT hr = device->CreateBuffer(&desc, 0, dofConstantBuffer.GetAddressOf());
		_ASSERT_EXPR(SUCCEEDED(hr), HRTrace(hr));
	}
...
// 描画開始
void DepthOfFieldShader::Begin(const RenderContext& rc)
{
...
// 描画
void DepthOfFieldShader::Draw(const RenderContext& rc, const Sprite* sprite)
{
	//	定数バッファの情報を設定
	CBDepthOfField cbDepthOfField;
	cbDepthOfField.focusDistance = rc.depthOfFieldData.focusDistance;
	cbDepthOfField.dofRange = rc.depthOfFieldData.range;
	cbDepthOfField.near_clip = rc.depthOfFieldData.nearClip;
	cbDepthOfField.far_clip = rc.depthOfFieldData.farClip;
	rc.deviceContext->UpdateSubresource(dofConstantBuffer.Get(), 0, 0, &cbDepthOfField, 0, 0);
	...
	rc.deviceContext->PSSetShaderResources(1, 1, &rc.depthOfFieldData.bokehSceneSRV);
	rc.deviceContext->PSSetShaderResources(2, 1, &rc.depthOfFieldData.depthSRV);

	rc.deviceContext->Draw(4, 0);
}

// 描画終了
void DepthOfFieldShader::End(const RenderContext& rc)
{
