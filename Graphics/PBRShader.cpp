...
PBRShader::PBRShader(ID3D11Device* device)
{
	// 頂点シェーダー
	{
		// ファイルを開く
		FILE* fp = nullptr;
		fopen_s(&fp, "Shader\\PBRVS.cso", "rb");
	...
	// ピクセルシェーダー
	{
		// ファイルを開く
		FILE* fp = nullptr;
		fopen_s(&fp, "Shader\\PBRPS.cso", "rb");
		_ASSERT_EXPR_A(fp, "CSO File not found");
...
void PBRShader::Begin(const RenderContext& rc)
{
	...
	//	シャドウマップ設定
	rc.deviceContext->PSSetShaderResources(8, 1, &rc.shadowMapData.shadowMap);

	//	IBL情報
	rc.deviceContext->PSSetShaderResources(17, 1, &rc.iblData.diffuseIrradianceEnvironmentMap);
	rc.deviceContext->PSSetShaderResources(18, 1, &rc.iblData.specularPremappingRadianceEnvironmentMap);
	rc.deviceContext->PSSetShaderResources(19, 1, &rc.iblData.ggxLookUpTableMap);
}

// 描画
void PBRShader::Draw(const RenderContext& rc, const Model* model)
{
	...
	for (const ModelResource::Mesh& mesh : resource->GetMeshes())
	{
		...
		for (const ModelResource::Subset& subset : mesh.subsets)
		{
			...
			ID3D11ShaderResourceView* srvs[] =
			{
				...
				subset.material->metalness_smoothness_map.Get(),
				subset.material->occlusion_map.Get(),
			};
